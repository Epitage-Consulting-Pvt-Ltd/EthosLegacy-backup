...
class Fingerprint():
    ...
    def __init__(self, port, baud, timeout=1):
        ...
        self.ser = None
        self.last_deleted_id = None  # Track the last deleted ID
    ...
    def delete(self, idx=None):
        """Delete fingerprint by ID. If no ID is provided, delete all."""
        res = None
        if idx is None:
            # Delete all fingerprints
            res = self._send_packet("DeleteAll")
            ...
            if res:
                ack, _, _, _ = self._read_packet()
                if ack:
                    self.last_deleted_id = idx  # Store the last deleted ID
                return ack
        return None

    def enroll(self, try_cnt=10, sleep=1):
            """Enroll a fingerprint."""
            idx = None

            # Use the last deleted ID if available
            if self.last_deleted_id is not None:
                idx = self.last_deleted_id
            ...
            if idx is None or idx < 0:
                logger.error("No available ID for enrollment.")
                return -1
            ...
            for enr_num, enr in enumerate(["enroll1", "enroll2", "enroll3"]):
                logger.info(f"Start {enr}...")

                cnt = 0
                while not self.capture_finger():
                    cnt += 1
                    if cnt >= try_cnt:
                        logger.error("Failed to capture finger.")
                        return -1
                    time.sleep(sleep)
                    logger.info("Capturing a fingerprint...")
    ...
    def get_template(self, idx):
        """Get the fingerprint template for the specified ID."""
            ...
            if ack:
                ...
                if firstbyte == Fingerprint.PACKET_DATA_0 and secondbyte == Fingerprint.PACKET_DATA_1:
                    while True:
                        n = self.ser.inWaiting()
                        if n > 0:
                            packet = self.ser.read(n)
                            template.extend(packet)
                        else:
                            break
                return template
        return None

    def set_template(self, idx, template):
        """Set the fingerprint template for the specified ID."""
        if self._send_packet("SetTemplate", idx):
            ack, _, _, _ = self._read_packet()
            if ack:
                packet = bytearray([0x5A, 0xA5]) + template
                chksum = sum(packet)
                packet += [chksum & 0xFF, (chksum >> 8) & 0xFF]
                if self.ser and self.ser.writable():
                    self.ser.write(packet)
                    ack, _, _, _ = self._read_packet()
                    return ack
    ...
    def get_database(self):
        """Get the entire fingerprint template database."""
    ...
    def set_database(self, database):
        """Set the entire fingerprint template database."""
        if self._send_packet("GetDatabaseEnd"):
            ack, _, _, _ = self._read_packet()
            if ack:
                packet = bytearray([0x5A, 0xA5]) + database
                chksum = sum(packet)
                packet += [chksum & 0xFF, (chksum >> 8) & 0xFF]
                if self.ser and self.ser.writable():
                    self.ser.write(packet)
