...
#         self.fps_verify_thread = None
#         self.fps_stop_thread = None
#         self.fps_id = None

#     def verify_fps(self):
#         try:
#             # while not self.fps_stop_thread.is_set():
#             if self.f.init():
#                 print("Verify Fingerprint: %s" % str(self.f.identify()))
#                 self.f.close_serial()
#                 sleep(0.5)
#         except:
#             print("cleaning up fps")
#             self.cleanup_fps()
#             raise
...
class VerifyFingerprint:
    ...
    def __init__(self, db_path, port='COM5', baudrate=115200):
        self.f = Fingerprint(port, baudrate)
        # self.fps_verify_thread = None
        # self.fps_stop_thread = None
        self.fps_id = None
    ...
    def verify_fps(self):
        try:
            idtemp = self.f.identify()
            if self.f.capture_finger():                      #capture_finger function
                if idtemp == -1:
            ...
            # with sqlite3.connect(self.db_path) as conn:
                # fp_id = self.f.identify()
                # cursor = conn.cursor()
                # cursor.execute("SELECT fps_id FROM finger_point_scan WHERE fp_id = ?", (fp_id,))
                # fps_ids = cursor.fetchall()
                # matched_fps_ids = [fps_id[0] for fps_id in fps_ids if fps_id[0] == self.fps_id]
    ...
    def cleanup_fps(self):
        ...
        # pass
def main():
    handler = VerifyFingerprint(port='COM5', baudrate=9600)
    handler.verify_fps()
