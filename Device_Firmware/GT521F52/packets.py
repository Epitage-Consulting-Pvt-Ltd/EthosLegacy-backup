...
class response_packet(packet):
    def __init__(self, received_bytes):
        ...
        if len(received_bytes) != 12:
            raise ValueError(f"Expected 12 bytes, but received {len(received_bytes)} bytes")
...
class data_packet(packet):
    ...
    def mount_packet(self):
        packet_bytes = bytearray(len(self.data) + 6)
        ...
        packet_bytes[4:] = self.data
        
        self.checksum = self.checksum_calc(packet_bytes)
        
        packet_bytes[-2] = self.rightShift(self.checksum, 0)
        packet_bytes[-1] = self.rightShift(self.checksum, 8)
